...
std::array<GLfloat, 3 * (8 + 2)> vertices;
std::array<int, (8 + 1) * 2 * 3> triangles;
...
void Display()
{
	...
	if (normals == NORMALS_SMOOTH)
	{
		for (int i = 0; i < triangles.size() / 3; i++)
		{
			...
			//uśredniony wektor normalny jest normalizowany tylko, gdy biblioteka obsługuje automatyczne skalowania jednostkowych wektorów normalnych
			if (rescale_normal == true)
				Normalize(n);
			glNormal3fv(n);
			glVertex3fv(&vertices[3 * triangles[3 * i + 0]]);

			n[0] = n[1] = n[2] = 0.0; //obliczanie wektora normalnego dla drugiego wierzchołka
			...
			n[0] = n[1] = n[2] = 0.0; //obliczanie wektora normalnego dla trzeciego wierzchołka
...
void MouseButton(int button, int state, int x, int y)
{
	...
	{
		button_state = state; //zapamiętanie stanu lewego przycisku myszki
...
int main(int argc, char *argv[])
{
	...
	glutCreateWindow("Wektory normalne"); //utworzenie głównego okna programu
	...
	glutSpecialFunc(SpecialKeys); //dołączenie funkcji obsługi klawiszy funkcyjnych i klawiszy kursora
	...
	int MenuMaterial = glutCreateMenu(Menu); //utworzenie podmenu - Materiał
