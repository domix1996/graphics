...
void GenerateTextures()
{
	...
	// wczytanie tekstury texture.tga
	GLboolean error = load_targa("texture.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku texture.tga");
		exit(0);
	}
	...
	error = load_targa("texture.tga", width, height, format, type, pixels);
...
int main(int argc, char *argv[])
{
	...
	glutAddMenuEntry("texture.tga (kompresja)", TEXTURE_LENA);
	glutAddMenuEntry("texture.tga (bez kompresji)", TEXTURE_LENA_UNC);
